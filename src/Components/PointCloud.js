...
const PointCloud = () => {
  ...
  if (gotDataOd && gotDataPC){
    ...
    return (
      <div className="card" id="PointCloud-card">
        <h3 className="card-title-error" title={`${gotDataOd ? "" : "No Odometry reading!"} ${gotDataPC ? "" : "| No Point Cloud reading!"}`}> Lidar {gotDataOd? "" : "| No Odometry"} {gotDataPC? "" : "| No Pointcloud"}</h3>
        <div
          ref={mountRef}
          style={{
            width: "100%",
