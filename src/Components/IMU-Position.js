...
const IMU_Pos = () => {
  ...
  useEffect(() => {
    ...
    cameraRef.current.position.set(0.7, 1.5, 4.2);
    ...
    if (!vehicleRef.current) {
      ...
      roof.position.set(0, 0.6, -0.1);
      vehicle.add(roof);

      //Wheels
      const wheelGeometry = new THREE.CylinderGeometry(0.2, 0.2, 0.4, 12); 
      ...
      if (!controlsRef.current) {
        ...
        //Disable panning along the screen
  ...
  return (
    <div>
      <p className="IMU-label">Orientation</p>
      <div
      ref={mountRef}
      style={{
        width: "100%",
